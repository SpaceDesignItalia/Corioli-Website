  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {articoli.map((articolo) => (
        <article
          key={articolo.slug}
          className="relative flex flex-col bg-white rounded-2xl shadow-soft border border-gray-100 hover:shadow-card hover:-translate-y-1 transition-all duration-300 group overflow-hidden"
        >
          <Link
            href={`/blog/${articolo.slug}`}
            className="absolute inset-0 z-10"
            aria-label={`Leggi: ${articolo.titolo}`}
            onClick={() =>
